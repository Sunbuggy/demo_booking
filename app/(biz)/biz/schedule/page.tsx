...
// - Resource Planning: Daily/Weekly totals per Location & Dept
// - "Sticky" Shift Defaults: Remembers last used time/role per employee for fast entry
// ============================================================================
...
// NEW: Interface to remember the last used settings for an employee
...
export default function RosterPage() {
  ...
  // NEW: Store last used shift parameters for each employee
  // Key: EmployeeID, Value: { start, end, role, location }
  ...
  const handleCellClick = (emp: Employee, dateStr: string, existingShift?: Shift) => {
    ...
    if (existingShift) {
      ...
      // FEATURE: Check if we have "sticky" defaults for this employee
  ...
  const handleSaveShift = async () => {
    ...
    // FEATURE: Update sticky defaults for this employee
  ...
  const handleSaveProfile = async () => {
      ...
      const { error } = await supabase.from('users').update({ hire_date: profileEmp.hire_date, location: profileEmp.location, department: profileEmp.department, timeclock_blocked: profileEmp.timeclock_blocked, phone: profileEmp.phone, avatar_url: profileEmp.avatar_url }).eq('id', profileEmp.id);
  ...
  const groupedEmployees = useMemo(() => {
      ...
      Object.keys(LOCATIONS).forEach(loc => {
          groups[loc] = {}; LOCATIONS[loc].forEach(dept => { groups[loc][dept] = []; });
      ...
      shifts.forEach(shift => {
          ...
          if (shift.location && shift.location !== emp.location && groups[shift.location]) {
              const targetLoc = shift.location; const visitingBucket = groups[targetLoc]['Visiting Staff'];
      ...
      <div className="flex-1 overflow-auto border rounded-lg shadow-sm bg-card">
        ...
        <table className="w-full border-collapse min-w-[1000px] text-sm">
          ...
          <tbody>
            {Object.entries(groupedEmployees).map(([locName, departments]) => {
                ...
                // Shifts specific to this Location
                ...
                return (
                    ...
                    {/* Location Header Row with DAILY TOTALS */}
                    <tr className="bg-slate-900 text-white">
                        <td className="p-2 font-bold uppercase tracking-wider text-xs border-b border-slate-700 sticky left-0 z-10 bg-slate-900">
                            <div className="flex justify-between items-center">
                                ...
                                <span className="bg-slate-800 text-[10px] px-1.5 py-0.5 rounded text-slate-200">{sumWeeklyHours(locShifts).toFixed(1)}h</span>
                            </div>
                        </td>
                        {weekDays.map(day => {
                            const dailyTotal = sumDailyHours(locShifts, day.format('YYYY-MM-DD'));
                            return (
                                <td key={day.toString()} className="border-l border-slate-700 bg-slate-900 text-center text-[10px] font-mono text-slate-400">
                    ...
                    <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 h-12">
                        <td className="p-2 text-[10px] font-semibold text-muted-foreground uppercase border-r sticky left-0 z-10 bg-slate-50 dark:bg-slate-900">Forecast</td>
                        {weekDays.map(day => {
                            const dateStr = day.format('YYYY-MM-DD');
                            const dayData = locWeather.find(w => w.date === dateStr);
                            return (<td key={dateStr} className="border-l p-0 text-center"><WeatherCell data={dayData} /></td>);
                        })}
                    </tr>
                    ...
                    {Object.entries(departments).map(([deptName, deptEmps]) => {
                        ...
                        // Filter shifts specifically for this department
                        ...
                        return (
                            ...
                            {/* Department Header with DAILY TOTALS */}
                            <tr className={isVisiting ? 'bg-amber-50 dark:bg-amber-950/30' : 'bg-slate-100 dark:bg-slate-800'}>
                                <td className={`p-1 pl-4 font-semibold text-xs uppercase border-b sticky left-0 z-10 ${isVisiting ? 'text-amber-600' : 'text-slate-500'}`}>
                                    <div className="flex justify-between items-center pr-2">
                                        ...
                                        {/* Weekly Total for Dept */}
                                        <span className="text-[10px] opacity-70 bg-black/5 dark:bg-white/10 px-1 rounded">{sumWeeklyHours(deptShifts).toFixed(1)}h</span>
                                    </div>
                                </td>
                                {weekDays.map(day => {
                                    const dailyDeptTotal = sumDailyHours(deptShifts, day.format('YYYY-MM-DD'));
                                    return (
                                        <td key={day.toString()} className="border-l border-b bg-inherit text-center text-[10px] font-mono text-slate-500">
                            ...
                            {deptEmps.map(emp => {
                                ...
                                <tr key={`${locName}-${emp.id}`} className="hover:bg-muted/20 transition-colors border-b">
                                    <td className="p-0 border-r border-r-slate-100 dark:border-r-slate-800 sticky left-0 z-10 bg-card">
                                        <div className="p-2 w-64 cursor-pointer flex flex-row items-center h-full gap-3" onClick={(e) => { e.stopPropagation(); setProfileEmp(emp); setIsProfileModalOpen(true); }}>
        ...
        {viewMode === 'day' && (
            <div className="p-4 space-y-6">
                    ...
                    return (
                        <div key={locName} className="border rounded-lg overflow-hidden bg-card shadow-sm">
                            <div className="bg-slate-900 text-white p-3 font-bold uppercase flex justify-between items-center">
                                ...
                                {todayWeather && (<div className="flex items-center gap-2 bg-slate-800 px-3 py-1 rounded-full border border-slate-700"><WeatherCell data={todayWeather} /></div>)}
                            </div>
                                    ...
                                    <div key={deptName} className="p-0">
                                        ...
                                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                            {deptEmps.map(emp => {
                                                ...
                                                return (
                                                    <div key={emp.id} className="flex items-center justify-between p-3 hover:bg-muted/10 cursor-pointer" onClick={() => isManager && handleCellClick(emp, dateStr, shift)}>
                                                        <div className="flex items-center gap-3">
                                                            <UserAvatar emp={emp} isOnline={liveStatus[emp.id]} />
                                                            <div>
                                                                <div className="font-semibold text-sm flex items-center gap-2"><span className={isVisiting ? "italic text-amber-700 dark:text-amber-500" : ""}>{emp.full_name}</span><div onClick={(e)=>e.stopPropagation()}><ContactMenu user={emp} iconOnly={true} /></div></div>
                                                                <div className="text-xs text-muted-foreground flex items-center gap-1"><Badge variant="outline" className="text-[10px] h-5 px-1">{shift.role}</Badge></div>
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                            <div className="font-mono font-bold text-sm bg-blue-50 dark:bg-blue-900/50 text-blue-700 dark:text-blue-100 px-2 py-1 rounded border border-blue-100 dark:border-blue-900">{moment(shift.start_time).format('h:mm A')} - {moment(shift.end_time).format('h:mm A')}</div>
                ...
                }) && (<div className="text-center py-12 text-muted-foreground bg-card border rounded-lg border-dashed"><Clock className="w-10 h-10 mx-auto mb-2 opacity-20" /><p>No shifts scheduled for {currentDate.format('MMM Do')}</p><Button variant="link" size="sm" onClick={() => setViewMode('week')}>Switch to Week View</Button></div>)}
            </div>
        )}
      </div>

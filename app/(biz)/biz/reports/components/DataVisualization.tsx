...
const DataVisualization: React.FC<DataVisualizationProps> = ({
  ...
  // --- LOGIC: CSV Export ---
  const convertToCSV = (dataToExport: any[]) => {
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      {/* 1. HEADER CONTROLS */}
      {/* THEME FIX: bg-card, border-border */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-4 rounded-lg border border-border shadow-sm">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
           <Input
            ...
            // THEME FIX: bg-background, text-foreground
            className="w-full md:w-80 bg-background border-input"
          />
        </div>
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <Sheet>
            ...
            {/* THEME FIX: SheetContent will inherit global theme now */}
            <SheetContent>
              <SheetHeader>
              ...
              <div className="mt-4 grid grid-cols-1 gap-2 max-h-[80vh] overflow-y-auto">
                {columns.map((column) => (
                  <div key={column} className="flex items-center space-x-3 p-2 hover:bg-muted rounded">
                    <Checkbox
                      checked={visibleColumns.includes(column)}
                      onCheckedChange={() => toggleColumnVisibility(column)}
                      id={`col-${column}`}
                    />
                    <label htmlFor={`col-${column}`} className="text-sm cursor-pointer w-full font-medium">
          ...
          <Button onClick={downloadData} variant="outline" className="flex-1 md:flex-none">
      ...
      {/* 2. MAIN TABLE */}
      {/* THEME FIX: bg-card, border-border, shadow */}
      <div className="border border-border rounded-md bg-card shadow-sm w-full max-w-[calc(100vw-3rem)] md:max-w-full mx-auto">
        <div className="overflow-x-auto w-full">
          <Table className="min-w-full">
            {/* THEME FIX: bg-muted/50 for header */}
            <TableHeader className="bg-muted/50">
              <TableRow className="border-border hover:bg-transparent">
                {visibleColumns.map((column) => (
                  <TableHead key={column} className="text-muted-foreground font-semibold whitespace-nowrap px-4">
                    <button 
              ...
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((row, i) => {
                  ...
                  return (
                    <TableRow 
                      key={i} 
                      // THEME FIX: Use red-100/red-900 based on mode for errors, hover-muted for rows
                    ...
                    >
                      {visibleColumns.map((col) => (
                        <TableCell key={`${i}-${col}`} className="py-3 px-4 text-foreground whitespace-nowrap">
                          {col === 'amount' ? (

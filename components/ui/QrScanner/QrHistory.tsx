...
export const QrScanHistory = ({ user }: { user: UserType | null }) => {
  ...
  const fetchUserScanHistory = async () => {
    ...
    const { data, error } = await supabase
      .from('qr_history')
      .select('id, link, scanned_at, location, latitude, longitude') // Fetch latitude and longitude
      .eq('user', user.id)
    ...
    if (error) {
      ...
      const formattedData = data.map((record: any) => ({
        ...
        latitude: record.latitude ?? null,  // Add latitude
        longitude: record.longitude ?? null, // Add longitude
      }));
